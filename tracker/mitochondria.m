...
%   NUMIMAGES is the numbers of the first and last picture. of the form [first, last] 
%   where first and last are both integers. INCR is value that the picture
%   number will increment by. HERTZ is the number of seconds per pictures. 
%   RATIO is the number of microns per pixel edge,
...
%List of picture numbers to be considered
        ...
        for ii = 1:max(L(:))
            ...
            if (any(L(overlaps) == ii))
                ...
                if (~ismember(ii,specialobjects))                
                    ...
                    %NOTE: find is used instead of logical indexing because the
...
netdistances = netdistances(lifetime>=lifetimemin);
    ...
    function plot_centroid
        ...
        %converts the lifetime min from seconds to frames
